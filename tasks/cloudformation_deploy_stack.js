...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask("cloudformation_deploy_stack",
    "uploads files to S3 and creates a new stack with file versions as parameters.",
    function() {
      var done = this.async();
      ...
      async.series([
        ...
        function(callback) {
          createCloudformation(data, callback);
        }
      ], function (err) {
        done(!err);
      });
  ...
  function createCloudformation(data, callback) {
    var params = {
    ...
    cloudformation.createStack(params, function(err, data){
      if (err) {
        ...
        grunt.log.writeln("Create stack:\n" + JSON.stringify(data,null,1));
        callback(null);
      }
    });
  }
